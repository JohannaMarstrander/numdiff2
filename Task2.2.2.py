# -*- coding: utf-8 -*-
"""
Created on Wed Mar 11 08:43:01 2020
...
def solve(p, A_star, F_star, ustar):
    ...
    for n in range(p.N):
        ...
        S[n + 1, 1:-1] = SU_star + p.k / 2 * (p.fS(SU_star,I[n, 1:-1], p.params) - p.fS(S[n, 1:-1], I[n, 1:-1], p.params))
        I[n + 1, 1:-1] = IU_star + p.k / 2 * (p.fI(IU_star, S[n, 1:-1], p.params) - p.fI(I[n, 1:-1], S[n, 1:-1], p.params))
...
def F_star(r,k, U, u_star0, u_star1, f, params, other):
    F = r / 2 * U[:-2] + (1 - r) * U[1:-1] + r / 2 * U[2:] + k * f(U[1:-1], other[1:-1], params)
    F[0] += r/2 * u_star0
    F[-1] += r/2 * u_star1
    return F   
